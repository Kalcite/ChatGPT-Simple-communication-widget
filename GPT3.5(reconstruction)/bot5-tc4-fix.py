...
os.system('')
# 替换为自己的API Key
print("\n\033[1;34m 常用指令：\nedit：生成log.txt，方便查阅聊天记录\nopenlog：打开log.txt，查看聊天记录\ndellog：删除log.txt并退出程序(清空聊天记录)，下次启动程序时再次生成\nghpai：进入图片ai模式，根据你的需求生成图片并返回图片地址\nquit：退出程序 \033[0m")
...
def send_ghprequest():
    try:
        ...
        return image_url
        # message_log.append({"role":"assistant","content":image_url})
        input()
...
def main():
    ...
    while True:
        ...
        if user_input == "quit":
            ...
            message_log = read_log('log.txt')
            main()
        elif user_input == 'ghpai':
            print("DALL-E 2 API")
            send_ghprequest()
            print("Press any key to continue")
            message_log.append({"role": "assistant", "content": send_ghprequest})
            save_log(message_log)
            input()
            main()
            ...
            save_log(message_log)
            input("Open your log file now , Press an key to continue\n")
            openlog('log.txt')
