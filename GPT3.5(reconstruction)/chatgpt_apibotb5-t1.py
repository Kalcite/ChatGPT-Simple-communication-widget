...
def main():
    ...
    while True:
        ...
        if user_input == "quit":
            ...
            save_log
            message_log = dellog('log.txt')
        elif user_input == "cx":
            save_log(message_log)
            url = f'https://api.openai.com/v1/billing'
            response = requests.get(url, headers={'Authorization': f'Bearer {apikey_set}'})
            if response.status_code == 200:
                data = response.json()
                print(f'Your current balance is: {data["data"][0]["balance"]}')
            else:
                print(f'Error occurred while checking balance. {response.content}')
        else:
